...
# =============================================================================
# Performance Optimization Configs (DRAM-Sharded, Compute Kernels)
# =============================================================================

def create_dram_sharded_mem_config(k_dim: int, n_dim: int) -> "ttnn.MemoryConfig":
    """
    Create DRAM-sharded memory config for weight matrices.

    Distributes weights across 12 DRAM banks for ~26% bandwidth improvement
    (190 GB/s → 240 GB/s).

    Args:
        k_dim: K dimension of weight matrix (input features)
        n_dim: N dimension of weight matrix (output features)

    Returns:
        ttnn.MemoryConfig with WIDTH_SHARDED on DRAM
    """
    import ttnn

    num_banks = 12  # Blackhole has 12 DRAM banks

    # Ensure n_dim is divisible by num_banks
    n_per_bank = (n_dim + num_banks - 1) // num_banks
    n_padded = n_per_bank * num_banks
...
def get_matmul_program_config(
    m: int, k: int, n: int, num_cores: int = 32
) -> "ttnn.MatmulMultiCoreReuseMultiCastDRAMShardedProgramConfig":
    """
    Get DRAM-sharded matmul program config for decode operations.

    Args:
        m: M dimension (batch size for decode, typically 1)
        k: K dimension (input features)
        n: N dimension (output features)
        num_cores: Number of cores to use (default 32 for Blackhole)
    ...
    return ttnn.MatmulMultiCoreReuseMultiCastDRAMShardedProgramConfig(
        in0_block_w=max(1, k // tile_size // num_cores),
