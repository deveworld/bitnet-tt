...
# =============================================================================
# HEIGHT_SHARDED Memory Configs for batch_size=32 optimization
# =============================================================================

def get_decode_memory_configs(head_dim: int = 128, batch_size: int = 32) -> dict:
    """
    Get HEIGHT_SHARDED memory configs for optimized decode path.

    Based on tt_transformers/model_config.py for Blackhole p150a.
    Requires batch_size=32 to match CoreGrid(y=4, x=8) = 32 cores.

    Args:
        head_dim: Attention head dimension (default 128)
        batch_size: Batch size (must be 32 for HEIGHT_SHARDED)

    Returns:
        Dict with memory configs for decode operations
    """
    import ttnn

    assert batch_size == 32, f"HEIGHT_SHARDED requires batch_size=32, got {batch_size}"

    configs = {}

    # CREATE_QKV_DECODE_SHARD: Output config for nlp_create_qkv_heads_decode
    # Shape: [1, batch, heads, head_dim] -> shard per batch element
    configs["CREATE_QKV_DECODE_SHARD"] = ttnn.create_sharded_memory_config(
        shape=(ttnn.TILE_SIZE, head_dim),  # (32, 128)
        core_grid=ttnn.CoreGrid(y=4, x=8),  # 32 cores for batch=32
        strategy=ttnn.ShardStrategy.HEIGHT,
        orientation=ttnn.ShardOrientation.ROW_MAJOR,
        use_height_and_width_as_shard_shape=True,
    )

    # L1_HEIGHT_SHARDED for general HEIGHT_SHARDED operations
    configs["L1_HEIGHT_SHARDED"] = ttnn.L1_HEIGHT_SHARDED_MEMORY_CONFIG

    # DRAM config for KV cache storage
    configs["KV_CACHE_DRAM"] = ttnn.DRAM_MEMORY_CONFIG
