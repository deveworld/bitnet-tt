...
> **Snapshot** -- auto-generated by `scripts/pcc_localize.py --rank` on a
> single prompt (`"The capital of France is"`), single TT run. Fresh
> data can be regenerated with `BITNET_LOCALIZE=1 python3 scripts/pcc_localize.py --capture --rank`.
>
> **Key interpretation** (see `docs/STATUS.md` §2, `docs/plan_sdpa_kernel_alignment.md`):
> - L0.block_input RFE 0.000 -- embedding is bf16 bit-identical (sanity).
> - L0.post_input_norm RFE 0.009 -- RMSNorm itself is NOT the drift source.
> - L0.post_self_attn RFE 0.240, L0.post_attn_sub_norm 0.420 -- drift
>   enters the fused SDPA chain. Phase K kernel-alignment plan targets
>   this chain directly.
> - Later-layer post_input_norm (L5=0.385, L15=0.324) high RFE is
>   downstream amplification of L0's drift through RMSNorm's reciprocal-
>   sqrt nonlinearity, not a per-op L5/L15 issue.

Prompts: 1 (`"The capital of France is"`). Capture layers: [0, 5, 15, 25, 29].
