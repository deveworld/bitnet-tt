...
| 31. **cpu().to_list() argmax readout** | **85.5 p50 / 74.28 decode_tps** | Four per-step uint32 argmax readout sites (in `_sample_token` and `_on_device_argmax_single`) were going through `to_layout(TILE) + typecast(int32) + to_torch` (0.282 ms/call) because torch 2.2.2 dlpack rejects kUInt bit widths > 8. Replaced with `ttnn.Tensor.cpu().to_list()` (0.047 ms/call, 5.5× cheaper). Session 11+12 combined gain: +3.23 decode_tps on top of the post-dlpack baseline (71.05 → 74.28). Now exceeds the pre-dlpack-regression baseline. |

### Accuracy (current HEAD, multiple references)

Prompt `"The capital of France is"` → argmax ` Paris` on all references.
